#!/usr/bin/env bash
# Catch-up script: jump to the end-of-act-1 state (mgmt management cluster + CAPD ready, demo cluster gone)
# Idempotent: wherever you got stuck, this leaves a clean act-1 endpoint. About 5 minutes.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# the conversion webhook needs a few seconds after rollout; probe it with a server-side dry-run
