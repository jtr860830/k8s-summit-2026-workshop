# 第二幕：kro 自助服務 —— 一個物件開出一個叢集

> 這種「使用者從平台自助開通資源」的模式，就是常聽到的 service catalog。

第一幕那七個物件是「基礎設施工程師」的世界。這一幕你會把它包成**平台 API**，
...
第一幕七個物件 → 第二幕一個。差距不是魔法，是**平台工程**：
把專家知識（那些物件怎麼寫、哪些欄位危險）封裝成預設值與鎖死欄位，
