  ...
  <div style={{ ...fill, background: darkBg, color: '#fff', padding: 120, position: 'relative', display: 'flex', alignItems: 'center', gap: 96 }}>
    ...
    <div style={{ flex: 1 }}>
      ...
      <div style={{ marginTop: 44, fontSize: 30, lineHeight: 1.7, color: '#f0f0f0' }}>
        ...
        <div><span style={{ color: '#f0b5a1', fontWeight: 800 }}>擅長</span>　Linux · Kubernetes · Container · Rust</div>
...
const Claim: Page = () => (
  <Dark eyebrow="THESIS" title="用 K8s 管理整個基礎設施">
    <ul style={{ fontSize: 40, paddingLeft: 46, margin: 0, color: '#f0f0f0' }}>
      <Li gap={36}>同一套 kubectl、YAML、RBAC、GitOps <Red>管到底</Red></Li>
      <Li gap={36}>機器、叢集、上架規則<Red>都是 K8s 物件</Red></Li>
...
const WhiteBox: Page = () => (
  <Light eyebrow="WHY NOW" title="從裸機到自助服務，每一層都有成熟的開源專案">
    ...
    <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: '0.25em', color: 'var(--osd-accent)', marginTop: 48 }}>選型原則</div>
    <ul style={{ fontSize: 36, paddingLeft: 48, margin: '16px 0 0' }}>
      <Li gap={18}>只用<Red>開源授權</Red>的專案（CNCF 或社群治理）</Li>
      <Li gap={18}>每個元件都能<Red>單獨抽換</Red>，不用動到其他層</Li>
    </ul>
  </Light>
);
...
const Act1Recap: Page = () => (
  <Light eyebrow="ACT 1 · RECAP" title="好用，但太多細節">
    <div style={{ display: 'flex', gap: 40, marginTop: 10 }}>
      <div style={{ flex: 1, background: '#fff', border: '1px solid #e8e2df', borderRadius: 'var(--osd-radius)', padding: '40px 48px' }}>
...
const BootstrapFull: Page = () => (
  <Dark eyebrow="DAY-0 · WRAP-UP" title="起始機的作用">
    <ul style={{ fontSize: 38, paddingLeft: 46, margin: 0, color: '#f0f0f0' }}>
      ...
      <Li gap={28}>起始機可以是一台筆電：接上機房網段、跑完流程、pivot、關機。管理叢集不依賴它</Li>
