...
export const nLinks = `先把這個網址記下來：josh.tw/ks26。今天的簡報、教材、所有要打的指令都在裡面，等一下動手的時候直接開著照做，不用抄投影幕。`;
...
沒跑過的現在跑，要下載約 2 GB 的映像，看會場網速 5 到 20 分鐘。來不及的先聽，第一幕開始前有 reset 腳本可以追上。
...
workloadcluster 幾十秒內就會顯示 ACTIVE、READY True，那代表 kro 把底層七個物件都建好了；叢集本身要等 Machine 轉 Running，大概 4 分鐘。那七個 Cluster API 物件是 kro 建的，你沒有碰它們。`;
...
【兩個示範合起來講】Cluster API 的哲學是壞了就換，雲上成立是因為換一台很便宜。接下來兩個示範就是在裸機上把換一台的代價降下來：demo② 讓換機不用搬資料，demo③ 讓升級根本不用換機。這樣裸機才有資格用 Cluster API 的方式管。`;
...
【痛點】HCI 節點同時是運算節點也是儲存節點，作業系統壞了想重灌，Ceph 的資料跟著這台走。傳統做法是先把 OSD 踢出 Ceph、等資料搬到別台、重灌、加回來、再等資料搬回來。一台幾 TB，搬兩趟就是幾小時到一天，叢集一直降級。
【場景】節點 OS 壞了要重來；換作業系統版本或映像；換系統碟；想把 Cluster API 的換機哲學用在有狀態節點上，不因為上面有 Ceph 就特殊處理。
【一句話】讓儲存節點也能當 cattle，重灌只是重灌，不是搬家。`;
...
【痛點】Cluster API 預設升級是換機，雲上有無限機器可以開，裸機沒有。三台控制平面的機房要升級，得多養一台備機，或一台一台拆了重灌，又回到搬資料的問題。結果是裸機使用者不敢升級，版本一路落後。
【場景】每月一次的 patch 升級；機房沒有備機或不想為升級多養一台；節點有狀態，控制平面也跑 Ceph；升級窗口短，不能接受每台十幾分鐘重灌加重開。
【一句話】讓裸機的 Kubernetes 升級回到「改一個版本號，等它自己做完」，跟雲上一樣。`;
